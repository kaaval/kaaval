...
def _flatten_rules_for_subject(
    subject_bindings: list[dict],
    role_index: dict[tuple, dict],
) -> list[dict]:
    """Return all rules across every Role/ClusterRole in ``subject_bindings``.

    Accepts only the bindings already known to include this subject (built
    from subject_to_bindings[key]), so the scan is O(bindings-per-subject)
    rather than O(all-bindings * subjects-per-binding).
    """
    rules: list[dict] = []
...
def _build_privileged_namespaces(
    ...
    """
    Return the set of namespaces that contain at least one non-default
    ServiceAccount bound to a privileged role.

    Only concrete namespace strings are recorded — SA subjects with no
    ``namespace`` key are skipped entirely.  This prevents a missing field
    from inserting ``None`` into the set, which would otherwise cause the
    ``or None in privileged_namespaces`` guard in the caller to match every
    pod-creating subject regardless of namespace (false-positive flood).

    A real ServiceAccount always has a namespace in the Kubernetes API; the
    only time the field is absent is in hand-crafted or partially-rendered
    YAML passed to POST /combo-scan.  Skipping those subjects is the safe
    choice: we never widen the match to unrelated namespaces.
    """
    privileged_ns: set[str] = set()
    for binding in all_bindings:
        ...
        lookup_ns = binding_ns if role_kind == "Role" else None
        role = role_index.get((role_kind, lookup_ns, role_name))
        if not (role and _role_is_privileged(role.get("rules") or [])):
            continue
        for s in subjects:
            ...
            if sa_ns is None:
                # Skip: no concrete namespace — recording None would widen
                # the match globally, flagging every pod-creator in the graph.
                logger.debug(
                    "Skipping privileged SA %r in binding %r: no namespace field",
                    s.get("name"),
                    binding.get("name"),
                )
                continue
...
def evaluate_combo_findings(graph: dict, context: dict) -> list[dict]:
    ...
    for r in graph.get("roles", []):
        if r.get("name"):
            role_index[("Role", r.get("namespace"), r.get("name"))] = r
    for r in graph.get("cluster_roles", []):
        if r.get("name"):
            role_index[("ClusterRole", None, r.get("name"))] = r
    ...
    for subject_key, bindings in subject_to_bindings.items():
        ...
        aggregate_rules = _flatten_rules_for_subject(bindings, role_index)
        if not aggregate_rules:
            continue

        # Determine whether this subject's namespace has a privileged SA.
        # privileged_namespaces only contains concrete strings (never None), so a
        # subject with no namespace (subject_ns is None) correctly gets False here.
        has_privileged_sa = subject_ns is not None and subject_ns in privileged_namespaces
